...
# args = parser.parse_args(["txome_antisense_all.bam", "cb-umi_dict.pickle", "txome_antisense.bam"])
...
for read in in_bam:
    ...
    if read.has_tag("CB") and cb_umi_dict.get(read.get_tag("CB")) is not None and read.get_tag("UB") not in cb_umi_dict[read.get_tag("CB")]:
        # add the read to the output bam
        o = out_bam.write(read)
